...
//Si esto queda con const, no podra ser modificado desde el action, que es lo que deberia ser
let GruposRegex = {
	CARGO: { 
	...
	for (var i = 0, len = resp.included.length; i < len; ++i) {
		...
		switch (fila.$type) {
			...
			case 'com.linkedin.voyager.dash.jobs.JobPostingCard':
					...
					await chrome.storage.local.get(id_oferta).then((oferta) => {
						try {
							oferta[id_oferta].cantidad_solicitudes.push({
								cantidad_solicitudes : regx.buscar_match(fila.primaryDescription.text, /(\d+)(?= solicitud)/),
				...
				try {
					Lista_oferta[id_oferta].empresa = regx.buscar_match(fila.primaryDescription.text, /^([\s\.\wÀ-ÿ]*\b)/)
					...
					Lista_oferta[id_oferta].localizacion_empleo = regx.buscar_match(fila.primaryDescription.text, /(\b[\s\w,À-ÿ]*chile)/)
					Lista_oferta[id_oferta].tipo_modalidad = regx.buscar_match(fila.primaryDescription.text, /(híbrido|remoto|presencial)/)
